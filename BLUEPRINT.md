...
- **Market data lookup**: "Look Up" button on the Add Trade form (js/marketdata.js) queries Finnhub (stocks/forex/crypto) or Twelve Data (Futures/Future Options — Finnhub's free tier barely covers futures) for company name/exchange and the current live quote, with a one-click "Use as Entry Price" fill. Both are user-supplied free keys, Settings > Market Data. Free tiers only expose the current quote, not a historical price for a past trade date — genuinely free point-in-time futures/options data isn't available without a paid provider
